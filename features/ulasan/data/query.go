...
func (uq *ulasanQuery) GetAll() ([]ulasan.Core, error) {
	res := []AllUlasan{}
	err := uq.db.Raw("SELECT ulasans.id, ulasans.guru_id, gurus.nama as nama_guru, ulasans.penilaian, ulasans.ulasan FROM ulasans JOIN gurus ON ulasans.guru_id = gurus.id WHERE ulasans.deleted_at is NULL").Find(&res).Error
	if err != nil {
